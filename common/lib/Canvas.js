export default class Canvas {
    constructor(canvas, { axis = false, size, dash = 8, strokeStyle = 'black', fillStyle = 'black' } = {}) {
    ...
    point({ x, y }, name) {
        this.circle(x, y, 2, false)
        if (name) this.text(x, y, name, true)
    }
    polygon(points, { fillStyle = 'black', close = true, rule = 'nonzero', filled = false } = {}) {
        const { ctx } = this
        ctx.beginPath()
        ctx.moveTo(...points[0])
