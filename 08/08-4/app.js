...
for (let i = 1; i <= 4; i++) {
    const p = pentagon[0].rotate(i * Math.PI * 0.4)   // 凸多边形内角为：1 - （边数 - 2） / 边数
    pentagon.push(p)
}
// 五角星
...
for (let i = 0; i <= 3; i++) {
    const rotated = [...quarterCircle].map(x => {
        return x.rotate(i * Math.PI * -0.5).translate(...translation[i])
    })
